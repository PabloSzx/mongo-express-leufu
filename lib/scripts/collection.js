...
$document.ready(function() {
  $("#tabs").tab();
  if (
    document.location.href.indexOf("query=") >= 0 &&
  ...
  $.ajax({
    ...
    if (data === "Valid") {
      ...
      $("#document-modal-body")
        .parent()
        .append(
          '<div id="documentInvalidJSON" class="alert alert-danger"><strong>Invalid JSON</strong></div>'
        );
    }
  });
  return false;
  ...
  $.ajax({
    ...
    if (data === "Valid") {
      ...
      $("#index-modal-body")
        .parent()
        .append(
          '<div id="indexInvalidJSON" class="alert alert-danger"><strong>Invalid JSON</strong></div>'
        );
    }
  });
  return false;
...
function makeCollectionUrl() {
  ...
  return `${st.baseHref}db/${encodeURIComponent(
    st.dbName
  )}/${encodeURIComponent(st.collectionName)}/`;
}
...
$document.ready(function() {
  ...
  $(".deleteButtonCollection").tooltip({
    title:
      "Are you sure you want to delete this collection? All documents will be deleted.",
  });

  $tableWrapper.scroll(function() {
    const proximityToRightOfTable =
      $(".tableWrapper table").width() -
      $tableWrapper.scrollLeft() -
      $tableWrapper.width();
    document.getElementById("fadeToWhiteID").style.opacity =
      Math.min(Math.max(proximityToRightOfTable - 50, 50) - 50, 100) / 100;
  });
  ...
  $(".tooDamnBig").bind("click", function(e) {
    ...
    $.get(`${makeCollectionUrl()}${encodeURIComponent(_id)}/${prop}`, function(
      input
    ) {
      // Images inline
      if (
      ...
      if (
        ...
        input =
          '<audio controls style="width:45px;" src="' +
          input +
      ...
      if (
        ...
        (input.toString() === "[object Object]" ||
          input.toString().substr(0, 7) === "[object")
      ) {
  ...
  $(".deleteButtonCollection").on("click", function(event) {
    ...
    $("#confirm-deletion-collection")
      ...
      .one("click", "#delete", function() {
        ...
        if (
          $input.val().toLowerCase() === $input.attr("shouldbe").toLowerCase()
        ) {
  ...
  $importInputsFile.on("change", event => {
    ...
    axios
      .post(
        `${ME_SETTINGS.baseHref}db/${ME_SETTINGS.dbName}/import/${collection}`,
        data
      )
      .then(val => {
        console.log("val :", val);
