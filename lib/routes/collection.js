...
var routes = function(config, socket) {
  ...
  exp.viewCollection = function(req, res) {
    ...
    var defaultKey =
      config.defaultKeyNames &&
      config.defaultKeyNames[dbName] &&
    ...
    req.collection
      .find(query, projection, query_options)
            ...
            req.collection.count(query, null, function(err, count) {
              ...
              for (var i in items) {
                ...
                for (let prop in items[i]) {
                  if (
                    utils.roughSizeOfObject(items[i][prop]) >
                    config.options.maxPropSize
                  ) {
                    items[i][prop] = {
                      attribu: prop,
                      display: "*** LARGE PROPERTY ***",
                      humanSz: utils.bytesToSize(
                        utils.roughSizeOfObject(items[i][prop])
                      ),
                ...
                if (
                  utils.roughSizeOfObject(items[i]) > config.options.maxRowSize
                ) {
                  for (let prop in items[i]) {
                    if (
                      prop !== "_id" &&
                      utils.roughSizeOfObject(items[i][prop]) > 200
                    ) {
                      items[i][prop] = {
                        attribu: prop,
                        display: "*** LARGE ROW ***",
    ...
    if (query) {
      ...
      req.collection.drop(function(err) {
        ...
        req.updateCollections(req.db, req.dbName, function(err) {
          ...
          req.session.success =
            'Collection  "' + req.collectionName + '" deleted!';
          socket.emit("dropped_collection", {
            collection_name: req.collectionName,
            db_name: req.dbName,
          });
  ...
  exp.renameCollection = function(req, res) {
    ...
    req.collection.rename(name, function(err) {
      ...
      const collection_old_name = req.collectionName;
  ...
  exp.updateCollections = function(req, res) {
    req.updateCollections(req.db, req.dbName, function(err) {
      ...
      socket.emit("collections_updated", { db_name: req.dbName });
      res.redirect(res.locals.baseHref + "db/" + req.dbName);
    });
  };
  ...
  exp.importCollection = function(req, res) {
    ...
    if (
      keys.some(key => !req.files[key].data || !req.files[key].data.toString)
    ) {
    ...
    docs = _.compact(
      _.map(docs, ({ _id, ...value }) => {
        if (_id) {
          const oid = _.get(_id, "$oid");
          if (oid) {
    ...
    req.collection.insertMany(docs, { ordered: false }, (err, stats) => {
      if (err) {
      ...
      if (stats) {
        const n = _.get(stats, "result.n", _.get(stats, "nInserted", 0));

        socket.emit("documents_inserted", {
          n,
          db_name: req.dbName,
          collection_name: req.collectionName,
        });
        ...
        if (!err) {
          ...
          req.session.success = `${n} document(s) inserted`;
          res.status(200).send(`${n} document(s) inserted`);
        } else {
          console.log(
            `${n} document(s) inserted but with errors!: ${err.message}`
          );

          req.session.success = `${n} document(s) inserted but with errors!: ${
            err.message
          }`;

          res
            .status(200)
            .send(`${n} document(s) inserted but with errors!: ${err.message}`);
        }
      }
    });
  };
