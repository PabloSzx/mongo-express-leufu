...
module.exports = function(config, socket) {
  ...
  const configuredDatabaseRoutes = databaseRoute(config, socket);
  const configuredCollectionRoutes = collectionRoute(config, socket);
  const configuredDocumentRoutes = documentRoute(config, socket);
  const configuredGridFSRoute = gridFSRoute(config, socket);
