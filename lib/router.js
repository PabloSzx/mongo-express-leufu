...
const router = function(config) {
  console.log("socket hostname: ", process.env.SOCKET);
  const socket = io(process.env.SOCKET || "http://localhost", {
  ...
  socket.on("connect", () => {
    console.log("mongo-express-leufu connected via sockets!");
  });
  ...
  if (config.useBasicAuth) {
    appRouter.use(
      basicAuth(config.basicAuth.username, config.basicAuth.password)
    );
  }
  ...
  appRouter.all("*", function(req, res, next) {
    ...
    if (config.mongodb.admin !== true && mongo.databases[0] !== "admin")
      return next();
  ...
  appRouter.param("collection", function(req, res, next, id) {
    ...
    res.locals.collectionUrl = utils.buildCollectionURL(
      res.locals.baseHref,
      res.locals.dbName,
      id
    );
  ...
  appRouter.param("document", function(req, res, next, id) {
    if (id === "undefined" || id === undefined) {
      ...
      return res.redirect(
        res.locals.baseHref + "db/" + req.dbName + "/" + req.collectionName
      );
    }
  ...
  appRouter.param("bucket", function(req, res, next, id) {
    ...
    mongo.connections[req.dbName].collection(id + ".files", function(
      err,
      filesConn
    ) {
  ...
  // routes
  const configuredRoutes = routes(config, socket);
  ...
  appRouter.delete(
    "/:database",
    mongoMiddleware,
    configuredRoutes.deleteDatabase
  );
  appRouter.get(
    "/db/:database",
    mongoMiddleware,
    configuredRoutes.viewDatabase
  );
  ...
  appRouter.post(
    ...
    mongoMiddleware,
    configuredRoutes.importCollection
  );
  appRouter.get(
    "/db/:database/compact/:collection",
    mongoMiddleware,
    configuredRoutes.compactCollection
  );
  appRouter.get(
    "/db/:database/expArr/:collection",
    mongoMiddleware,
    configuredRoutes.exportColArray
  );
  appRouter.get(
    "/db/:database/expCsv/:collection",
    mongoMiddleware,
    configuredRoutes.exportCsv
  );
  appRouter.get(
    "/db/:database/reIndex/:collection",
    mongoMiddleware,
    configuredRoutes.reIndex
  );
  appRouter.post(
    "/db/:database/addIndex/:collection",
    mongoMiddleware,
    configuredRoutes.addIndex
  );
  appRouter.get(
    "/db/:database/export/:collection",
    mongoMiddleware,
    configuredRoutes.exportCollection
  );
  appRouter.get(
    "/db/:database/dropIndex/:collection",
    mongoMiddleware,
    configuredRoutes.dropIndex
  );
  appRouter.get(
    "/db/:database/updateCollections",
    mongoMiddleware,
    configuredRoutes.updateCollections
  );
  ...
  appRouter.post(
    "/db/:database/gridFS",
    mongoMiddleware,
    configuredRoutes.addBucket
  );
  appRouter.delete(
    "/db/:database/gridFS/:bucket",
    mongoMiddleware,
    configuredRoutes.deleteBucket
  );

  appRouter.get(
    "/db/:database/gridFS/:bucket",
    mongoMiddleware,
    configuredRoutes.viewBucket
  );
  appRouter.post(
    "/db/:database/gridFS/:bucket",
    mongoMiddleware,
    configuredRoutes.addFile
  );
  appRouter.get(
    "/db/:database/gridFS/:bucket/:file",
    mongoMiddleware,
    configuredRoutes.getFile
  );
  appRouter.delete(
    "/db/:database/gridFS/:bucket/:file",
    mongoMiddleware,
    configuredRoutes.deleteFile
  );

  appRouter.get(
    "/db/:database/:collection",
    mongoMiddleware,
    configuredRoutes.viewCollection
  );
  appRouter.put(
    "/db/:database/:collection",
    mongoMiddleware,
    configuredRoutes.renameCollection
  );
  appRouter.delete(
    "/db/:database/:collection",
    mongoMiddleware,
    configuredRoutes.deleteCollection
  );
  appRouter.post(
    "/db/:database",
    mongoMiddleware,
    configuredRoutes.addCollection
  );
  ...
  appRouter.post(
    "/db/:database/:collection",
    mongoMiddleware,
    configuredRoutes.addDocument
  );
  appRouter.get(
    "/db/:database/:collection/:document",
    mongoMiddleware,
    configuredRoutes.viewDocument
  );
  appRouter.put(
    "/db/:database/:collection/:document",
    mongoMiddleware,
    configuredRoutes.updateDocument
  );
  appRouter.delete(
    "/db/:database/:collection/:document",
    mongoMiddleware,
    configuredRoutes.deleteDocument
  );
