...
var routes = function(config, socket) {
  ...
  exp.viewBucket = function(req, res) {
    ...
    var statsAvgChunk = utils.bytesToSize(
      files.reduce((prev, curr) => prev + curr.chunkSize, 0) / files.length
    );
    var statsTotalSize = utils.bytesToSize(
      files.reduce((prev, curr) => prev + curr.length, 0)
    );
